...
var cubeModule = function () {
  ...
  function up(ev) {
    ...
    if (!this.isMove) return; // this.style.transform = `rotateX(${this.rotateY}deg) rotateY(${this.rotateX}deg)`;

    $this.css('transform', "rotateX(".concat(this.rotateY, "deg) rotateY(").concat(this.rotateX, "deg)"));
  }
  ...
  return {
    init: function init(isInit) {
      ...
      if (isInit) return;
      $cube.on('touchstart', down).on('touchmove', move).on('touchend', up);
...
var swiperModule = function () {
  ...
  return {
    init: function init() {
      var _init = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 1;

      $swiperBox.css('display', 'block');

      if (mySwiper) {
        mySwiper.slideTo(_init, 0);
        return;
      }

      mySwiper = new Swiper('.swiper-container', {
        direction: 'horizontal',
        //滑动切换的方向垂直和水平horizontal | vertical
        loop: true,
        //循环
        effect: 'coverflow',
        //切换方式
